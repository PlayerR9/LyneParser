...
// TokenNodeIterator is a pull-based iterator that iterates
// over the children of a TokenNode.
type TokenNodeIterator[T gr.TokenTyper] struct {
	parent, current *TokenNode[T]
}
...
type TokenNode[T gr.TokenTyper] struct {
	Parent, FirstChild, NextSibling, LastChild, PrevSibling *TokenNode[T]
	Status                                                  EvalStatus
	Token                                                   *gr.Token[T]
}
...
func (tn *TokenNode[T]) String() string {
	...
	builder.WriteString("TokenNode[Token=")

	if tn.Token == nil {
		builder.WriteString("<nil>")
	} else {
		builder.WriteString(tn.Token.GoString())
	}

	builder.WriteString(", Status=")
	builder.WriteString(tn.Status.String())
	builder.WriteRune(']')
...
func (tn *TokenNode[T]) LinkChildren(children []treenode.Noder) {
	...
	var valid_children []*TokenNode[T]
...
func (tn *TokenNode[T]) GetLeaves() []treenode.Noder {
	...
	stack := Stacker.NewLinkedStack[treenode.Noder](tn)
...
//
// Returns:
//   - []treenode.Noder: A slice of pointers to the children of the node iff the node is the root.
//     Nil otherwise.
//
// Example:
//
...
//
// Returns:
//   - *TokenNode[T]: A pointer to the newly created node. It is
//     never nil.
...
// same as the behaviors of the TokenNode.AddChild function.
//
// Parameters:
...
// Returns:
//   - []treenode.Noder: A slice of pointers to the children of the node.
func (tn *TokenNode[T]) delete_child(target *TokenNode[T]) []treenode.Noder {
